...
const createInvoice = async (req, res) => {
    try {
      const { invoice_name, services,invoice_address,payment_mode,advance_payment} = req.body;
      ...
      const sqlInvoice = "INSERT INTO invoice_data (invoice_name, user_id,invoice_address,payment_mode,advance_payment) VALUES (?,?,?,?,?)";
      const resultInvoice = await new Promise((resolve, reject) => {
        db.query(sqlInvoice, [invoice_name, user_id,invoice_address,payment_mode,advance_payment], (err, result) => {
          if (err) {
            reject(err);
          } else {
            resolve(result);
  ...
  const UpdateInvoiceName = async (req, res) => {
    try {
        ...
        const { newName,newAddress,newpaymentMode,newadavacePayment } = req.body; // Extracting new invoice name from req.body

        // Construct SQL query to update the invoice name
        const sql = "UPDATE invoice_data SET invoice_name = ? , invoice_address=?, payment_mode = ? , advance_payment = ? WHERE invoice_id = ?";

        // Execute the update query asynchronously
        await new Promise((resolve, reject) => {
            db.query(sql, [newName, newAddress, newpaymentMode, newadavacePayment , invoiceId], (err, results) => {
                if (err) {
                    reject(err);
                } else {
                    resolve(results);
