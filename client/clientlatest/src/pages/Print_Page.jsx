  ...
  useEffect(() => {
    const fetchImages = async () => {
      try {
        ...
        if (response.status === 200) {
          const { company_name_account_name ,company_name_account_ifsc,company_name_account_number,company_address } = response.data;
...
<i className="bi bi-arrow-return-left mx-1"></i>   Final Quotation
     ...
     <div className="">
    <button className="btn btn-success mt-3 btn-print" onClick={handleChangeHeaderFooter}>
            {" "}
          
             Add Company Template
        
          </button>
          <button className="btn btn-danger mt-3 mx-lg-3 mx-sm-0  btn-print" onClick={handleDelete}>
            {" "}
          
             Delete Company data 
        
          </button>
          <button className="btn btn-secondary mt-3   btn-print" onClick={handleUpdate}>
            {" "}
